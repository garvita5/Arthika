...
class OpenAIService {
  ...
  getMockResponse(question, language) {
    ...
    const amountMatch = question.match(/(\d{1,3}(?:,\d{3})*(?:,\d{3})*)/);
    const detectedAmount = amountMatch ? parseInt(amountMatch[1].replace(/,/g, '')) : 50000;
    
    const mockResponses = {
      'gold loan': {
        storyResponse: `Imagine you're planning a journey to a distant city. A gold loan is like taking a shortcut through a risky path. While it gets you there faster, you might lose your precious belongings if you can't repay on time. For a ₹${detectedAmount.toLocaleString()} gold loan, you'd typically pay ₹${Math.round(detectedAmount * 0.07).toLocaleString()}-${Math.round(detectedAmount * 0.15).toLocaleString()} annually in interest. The bank holds your gold as security, so if you miss payments, you could lose your family's precious jewelry.`,
        recommendedSteps: [
        ...
        tags: ["loan", "gold", "risk"],
        riskLevel: "medium",
        estimatedCost: `₹${detectedAmount.toLocaleString()}`,
      ...
      'home loan': {
        storyResponse: `Think of buying a home like planting a tree that will grow for generations. A ₹${detectedAmount.toLocaleString()} home loan for 20 years is like nurturing this tree - it requires consistent care (monthly EMIs of ₹${Math.round(detectedAmount * 0.008).toLocaleString()}-${Math.round(detectedAmount * 0.01).toLocaleString()}). While it seems expensive now, this tree will provide shelter and potentially grow in value over time.`,
        recommendedSteps: [
        ...
        riskLevel: "low",
        estimatedCost: `₹${Math.round(detectedAmount * 0.008).toLocaleString()}-${Math.round(detectedAmount * 0.01).toLocaleString()} monthly EMI`,
      ...
      'investment': {
        storyResponse: `Picture your money as seeds that can grow into a forest. A smart investment strategy is like planting different types of trees - some grow fast (equity), some provide steady shade (debt), and some protect against storms (gold). Start with ₹${Math.round(detectedAmount * 0.1).toLocaleString()} monthly SIPs and watch your forest grow over time.`,
        recommendedSteps: [
        ...
        riskLevel: "low",
        estimatedCost: `₹${Math.round(detectedAmount * 0.1).toLocaleString()} monthly SIP`,
      ...
      'savings': {
        storyResponse: `Think of savings like building a strong foundation for your house. Every brick (savings) you add makes your financial house stronger. Start with ₹${Math.round(detectedAmount * 0.2).toLocaleString()} monthly savings - this could grow to ₹${Math.round(detectedAmount * 15).toLocaleString()} in 10 years with compound interest. The key is consistency and starting early.`,
        recommendedSteps: [
        ...
        riskLevel: "very low",
        estimatedCost: `₹${Math.round(detectedAmount * 0.2).toLocaleString()} monthly`,
      ...
      'emergency': {
        storyResponse: `An emergency fund is like having a spare tire in your car - you hope you never need it, but you're glad it's there when you do. Aim to save 6 months of expenses (₹${Math.round(detectedAmount * 0.6).toLocaleString()} for most families). This fund should be easily accessible but separate from your regular spending.`,
        recommendedSteps: [
        ...
        riskLevel: "very low",
        estimatedCost: `₹${Math.round(detectedAmount * 0.6).toLocaleString()} total`,
      ...
      'retirement': {
        storyResponse: `Retirement planning is like planting a tree today that will provide shade for your future self. Even a small monthly investment of ₹${Math.round(detectedAmount * 0.1).toLocaleString()} can grow to ₹${Math.round(detectedAmount * 25).toLocaleString()} in 30 years with compound interest. Start early, invest regularly, and let time work its magic.`,
        recommendedSteps: [
        ...
        riskLevel: "low",
        estimatedCost: `₹${Math.round(detectedAmount * 0.1).toLocaleString()}-${Math.round(detectedAmount * 0.3).toLocaleString()} monthly`,
