    ...
    <footer className="w-screen left-0 bg-gradient-to-br from-white via-blue-50 to-blue-100 border-t border-blue-100 shadow-lg mt-8 py-3 px-2 sm:px-6 flex flex-col items-center gap-2 rounded-b-2xl min-h-[56px]">
      ...
      <div className="w-full flex flex-col md:flex-row justify-between items-center gap-4 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col items-start gap-1 flex-1 min-w-[120px]">
          <a href="https://wa.me/1234567890" target="_blank" rel="noopener noreferrer" className="hover:underline text-green-600 text-sm">WhatsApp Bot</a>
          <span className="flex items-center gap-1 text-sm"><Phone size={16} className="inline-block" /> IVR: <a href="tel:1800123456" className="text-primary-700 hover:underline ml-1">1800-123-456</a></span>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-row items-center gap-2 flex-1 justify-center w-full max-w-xs">
          <input
            ...
            placeholder={submitted ? 'Thank you for your feedback!' : 'Write to us / Feedback'}
            value={feedback}
        ...
        <div className="flex flex-col items-end gap-1 flex-1 min-w-[120px]">
          <Link to="/privacy-policy" className="hover:underline text-primary-700 text-sm">Privacy Policy</Link>
          <Link to="/about" className="hover:underline text-primary-700 text-sm">About</Link>
          <a href="mailto:support@arthika.com" className="hover:underline text-primary-700 text-sm">Contact</a>
        </div>
      </div>
      <div className="text-xs text-gray-500 mt-2 w-full text-center border-t border-gray-200 pt-2">&copy; {new Date().getFullYear()} Arthika. All rights reserved.</div>
    </footer>
  );
};
